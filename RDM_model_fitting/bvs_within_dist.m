...
for cdxx = 1:3 %iterate through context combinations
    ...
    for ridx = 1:n_rand_iter
        %cvec2 = Shuffle(cvec);
        cvec2 = cvec(randperm(length(cvec)));
...
% calculate p value
pv = repmat(ratio_pos, [size(rand_ratio_pos,1), 1])>rand_ratio_pos;
pv = 1 - sum(pv(:).')./(numel(pv(:)));
%plot
figure(), hold on,
subplot(3,1,1), histogram(squeeze(rand_ratio_pos), 'BinWidth', 0.05, 'EdgeColor','none')
